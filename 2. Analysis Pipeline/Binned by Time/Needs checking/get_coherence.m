...
if input.Tentry_longepoch == 1
    X_save_loc = '_longEpoch';
elseif input.T_DataDriven == 1
    X_save_loc = '_CohEffect';
end

% save data
X_save = ['data',X_det,X_regs,X_save_loc];
save(X_save);
    ...
    if input.time_freq == 1
        ...
        % figure 1 
          ...
          for i = 1:length(bands)
            ...
            if p_norm{i}<0.05
                [p{i},h,stat]=signrank(diffscores{i});
            else
                [h,p{i},ci,stat]=ttest(diffscores{i});
            end
