   ...
   % define and load some variables 
    ...
    for i = 1:2
        ...
        if input.modindex == 1
            ...
            clearvars -except temp_folder sessions Datafolders folder_names nn input ...
        ...
        elseif input.comodgram == 1
            ...
            clearvars -except temp_folder sessions Datafolders folder_names nn input ...
               ...
               data_normAmp_sample data_phaseAxis_sample data_phaseAxis_choice M_idx
        end
    end
    ...
    % store session to track who you're analyzing
    sessions{nn-2} = temp_folder;
     
    % house-keeping
        clearvars -except Datafolders folder_names nn input ...
                   correct_trajectory frequencies data_MI_sample ...
                   data_phase_sample data_MI_choice data_phase_choice ...
                   phase_map_sample phase_map_choice sessions M_idx
end
cd('X:\07. Manuscripts\In preparation\Stout - JNeuro\Data\1-27-2020 makeover')
%save('PAC_all.mat');
...
elseif input.comodgram == 1
    ...
    amplitude_freq_bins = 1;%signal_data.amplitude_bandpass(2)-signal_data.amplitude_bandpass(1);
    phase_freq_bins = 1;%signal_data.phase_bandpass(2)-signal_data.phase_bandpass(1);
    signal_data.phase_bandpass     = [5 9];
    signal_data.amplitude_bandpass = [40 100];    
    ...
    figure('color',[1 1 1])
    pcolor(M_idx{2}.PhaseAxis,amplitude_highpass,phase_sample); % x pos was M.PhaseAxis
    set(gca,'fontsize', 13);
    colormap(jet)
    colorbar   
    caxis([0.0548 0.0563])    
    ...
    figure('color',[1 1 1])
    pcolor(M_idx{2}.PhaseAxis,amplitude_highpass,phase_choice); % find M.phaseaxis
    set(gca,'fontsize', 13);    
    colormap(jet)
    colorbar
    %caxis([0.0552 0.0561])
    ...
    figure('color',[1 1 1])
    pcolor(M_idx{2}.PhaseAxis,amplitude_highpass,squeeze(mean(cat(3,phase_sample,phase_choice),3)))
    set(gca,'fontsize', 13);    
    colormap(jet)
    colorbar
    %caxis([0.0552 0.0561])
    ...
    % split by rat
    split = [1,12;13,20;21,27]; % manual for now
    
    for i = 1:size(split,1)
        session_sample{i} = phase_sample_matrix(:,:,split(i,1):split(i,2));
        session_choice{i} = phase_choice_matrix(:,:,split(i,1):split(i,2));
    end
    
    for i = 1:length(session_choice)
        phase_sample_session{i} = mean(session_sample{i},3);
        phase_choice_session{i} = mean(session_choice{i},3); 
    end

    for i = 1:length(session_choice)
        figure('color',[1 1 1],'Renderer', 'painters', 'Position', [30 30 400 300])
        pcolor(M_idx{2}.PhaseAxis,amplitude_highpass,phase_sample_session{i}); % x pos was M.PhaseAxis
        set(gca,'fontsize', 13);
        colormap(jet)
        colorbar   
        %caxis([0.0548 0.0563])    
        ...
        figure('color',[1 1 1],'Renderer', 'painters', 'Position', [30 30 400 300])
        pcolor(M_idx{2}.PhaseAxis,amplitude_highpass,phase_choice_session{i}); % x pos was M.PhaseAxis
        set(gca,'fontsize', 13);
        colormap(jet)
        colorbar   
        %caxis([0.0548 0.0563]) 
