...
min_delay  = 5;
max_delay = 15; % total amount of time allowed for coherence to be detected
                ...
                if contains(trialType(triali),[{'H'} {'L'}])
                    ...
                    while openDoor == 0
                        ...
                        if toc(tStart) > max_delay && openDoor == 0
                            ...
                            delay_duration_notMet(triali) = toc(tStart);
                            if contains(trialType(triali),[{'H'} {'L'}])
                                delay_duration_notMet(triali) = delay_duration_notMet(triali); % a variable used to manipulate the control
                            end                        
