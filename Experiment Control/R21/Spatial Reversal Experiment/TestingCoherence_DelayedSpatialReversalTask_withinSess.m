...
if contains(testingType,[{'E'} {'e'}])
    ...
    % determine whether the condition is between->within or memory->within
    if contains(testingDay1,[{'Y'} {'y'}])
        rng('shuffle');
        randDay = randsample([1,2],1);
        if randDay == 1
            testingCond='MW'; % memory of previous day -> within reversal
        elseif randDay == 2
            testingCond='BW'; % between reversal -> within reversal
        end
    else
        prevTesting = load(data2load,'testingCond');
        prevTestingCond = prevTesting.testingCond;  
        if contains(prevTestingCond,'MW')
            testingCond = 'BW';
        elseif contains(prevTestingCond,'BW')
            testingCond = 'MW';
        elseif contains(prevTestingCond,'NA')
            error('Did not enter information correctly. hit Run and try again')
        end
    end
...
elseif contains(testingType,[{'R'} {'r'}])
    randDay = randsample([1,2],1);
    if randDay == 1
        traj = 'R';
    elseif randDay == 2
