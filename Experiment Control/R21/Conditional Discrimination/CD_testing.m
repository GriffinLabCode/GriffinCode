...
%}
wallInfo = input('Did you have stem walls in this session? ','s');
if contains(wallInfo,[{'Y'} {'y'}])
    wallInfo = 'Walls';
else
    wallInfo = 'noWalls';
end
place2store = ['X:\01.Experiments\R21\',targetRat];
cd(place2store);
save_var = strcat(rat_name,'_',task_name,'_',wallinfo,'_',eibSave,'_',c_save,'_CLEANED');
save(save_var);

place2store = ['X:\01.Experiments\R21\',targetRat];
cd(place2store);
save_var = strcat(rat_name,'_',task_name,'_',wallinfo,eibSave,'_',c_save,'_filtered');
