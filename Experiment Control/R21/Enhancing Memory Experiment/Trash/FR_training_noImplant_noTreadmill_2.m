...
while toc(sStart)/60 < session_length || sessEnd == 0
    ...
    for triali = 1:numTrials-1
        ...
        if triali == numTrials || toc(sStart)/60 > session_length || contains(trajectory{triali},'E')
            writeline(s,doorFuns.closeAll)
            sessEnd = 1;            
            break % break out of for loop
        end
...
place2store = ['X:\01.Experiments\R21\Experimental Cohort\FR Training Data - No Implant\', targetRat];
cd(place2store);
save(save_var);
