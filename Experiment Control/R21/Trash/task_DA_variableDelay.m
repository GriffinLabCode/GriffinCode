...
% using cell arrays (doesn't matter how many arrays are contained here),
% define what delay lengths to use. You could do as few as 1 cell array, to
% as many as you desire (for example delay = [0 5 30]; would contain trials
% that have either 0 sec, 5 sec, or 30 sec delay lengths)
...
for j = 1:length(delayLengths)
    delay{j} = repmat(delayLengths(j),[permTrials/4 1]);
end
...
% define a variable that tells us the number of unique trial types
for i = 1:length(delay)
    trialTypes(i) = delay{i}(1);
end
...
for triali = 1:numTrials
    ...
    if delay_length > 1
        ...
        % using the flexible delay variable, pause dependeding on the trial
        % type
        for j = 1:length(trialTypes)
            if trial_type(triali) == trialTypes(j)
                disp(['Pausing for ', num2str(trialTypes(j))])
                pause(trialTypes(j));
                break
            end
        end
