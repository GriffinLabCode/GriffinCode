...
%% set up connection with cheetah
...
for triali = 1:numTrials
    ...
    while next == 0   
        ...
        Y = [Y double(locationArray(2:2:length(locationArray)))];
        X = [X double(locationArray(1:2:length(locationArray)))];

        % get IdPhi using X and Y data
        % correct for tracking errors that result in zeroes
        VT_data.ExtractedX = X; VT_data.ExtractedY = Y;
        [X, Y] = interp_missing_VTdata(VT_data);
        IdPhi = IdPhi_RedishFun(X,Y,window_sec,postSmoothing,vt_srate,display);
        
        disp(['IdPhi = ',num2str(IdPhi)])

        if triali > 1 
            % turn on laser
            disp('TURNING ON LASER - PEW PEW')
        else
        ...
        if readDigitalPin(a,irArduino.rGoalArm)==0
            ...
            if triali == 1 && trajectory_text{triali} == 'R'
                ...
                % neuralynx timestamp command
                [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "RewardDelivered" 303 2');                
            elseif triali > 1 && trajectory_text{triali} == 'R' && trajectory_text{triali-1} == 'L'
                ...
                % neuralynx timestamp command
                [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "RewardNotDelivered" 304 2');                
            end
        ...
        elseif readDigitalPin(a,irArduino.lGoalArm)==0
            ...
            % Reward zone and eating
            % send to netcom 
            if triali == 1 && trajectory_text{triali} == 'L'
                for rewardi = 1:pellet_count
                    %pause(0.25)
...
% neuralynx timestamp command
[succeeded, cheetahReply] = NlxSendCommand('-PostEvent "SessionEnd" 701 3');
