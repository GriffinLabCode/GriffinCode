...
while redo == 1
    ...
    sample_traversal = cellstr(both_shuffled);

    % no more than 3 turns in one direction
    idxL = double(contains(sample_traversal,'L'));
    idxR = double(contains(sample_traversal,'R'));

    for i = 1:length(sample_traversal)-3
...
while toc(sStart)/60 < session_length || sessEnd == 0
    ...
    for triali = 1:numTrials
        ...
        if delay_length > 1
            ...
            [coh_trial{triali},coherence_met(triali),timeConv{triali},lfp_trial{triali},times_trial{triali}] = rat21_8(LFP1name,LFP2name,threshold,parammaze,tStart,doorFunmaze,maze,baselineMean,baselineSTD);
