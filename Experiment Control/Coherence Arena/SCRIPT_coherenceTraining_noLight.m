...
while toc(sStart)/60 < sessionDur 
    ...
    if isempty(rewLatency)==0
        probeTrialIdx = floor((numel(rewLatency)+1)/10) == ceil((numel(rewLatency)+1)/10);
    else
        probeTrialIdx = 0;
    end
    
    % trial idx for easier indexing of rewLatency
    trialIdx = horzcat(trialIdx,probeTrialIdx); % whenever trialIdx = 1, it means the trial was a probe
            ...
            if cohTheta > cohDelta && percSat < noisePercent && cohTheta > cohHighThreshold
                ...
                while rewExit == 0
                    ...
                    if readDigitalPin(a,arduinoIR)==0
                        ...
                        elseif probeTrialIdx == 1
                            [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "NosePokeProbe" 102 2');
                        end                            
                        % turn off the light
                        %writeDigitalPin(a,arduinoLED,0) 
                        % break out of loop
