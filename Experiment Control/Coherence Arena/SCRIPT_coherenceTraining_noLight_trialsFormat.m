...
% generate probe trial variable. Probe trials will comprise 10% of all
% trials and will be pseudorandomly presented (e.g. 1/10 trials)
loopTrials=1:10:numTrials;
rng('shuffle')
for i = 1:length(loopTrials)-1
    probeTrials(i) = randsample(loopTrials(i):loopTrials(i+1)-1,1);
end
...
while isempty(trialNum) || trialNum(end) < numTrials || toc(sStart) < sessionDur
    ...
    if isempty(trialNum)
        ...
        % because trialNum is determined after the trial ends,
        % determination of probe trial needs to occur before the trial
        % begins
        if isempty(find(probeTrials == trialNum(end)+1))
            probeTrialIdx = 0;
        else
            probeTrialIdx = 1;
        end
    ...
    % trial idx for easier indexing of rewLatency
    trialID = horzcat(trialID,probeTrialIdx); % whenever trialIdx = 1, it means the trial was a probe
    ...
    if isempty(starterVar)==0
        try
            ...
            if cohTheta > cohDelta && percSat < noisePercent && cohTheta > cohHighThreshold
                ...
                % deliver reward if not probe
                if probeTrialIdx == 0
                    writeline(s,rewFuns.right) 
                    rewTime = tic;
                    [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "RewardDeliveredTrue" 102 2'); 
                elseif probeTrialIdx == 1
                    rewTime = tic;
                    [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "RewardNotDeliveredProbe" 102 2'); 
                end
                ...
                while rewExit == 0
                    ...
                    if readDigitalPin(a,arduinoIR)==0
                        rewLatency = horzcat(rewLatency, toc(rewTime));
                        % send TTL to cheetah when rat pokes head into reward zone
                        if probeTrialIdx == 0 
                            [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "NosePokeReward" 102 2');
                        elseif probeTrialIdx == 1
                            [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "NosePokeProbeHit" 102 2');
                        end                            
                        % turn off the light
                        %writeDigitalPin(a,arduinoLED,0) 
                        % break out of loop
                    ...
                    % if after 30s, the rat hasnt poked, move on. This is
                    % only relevant for probe trials
                    if toc(rewOnset4probe) > 15 && probeTrialIdx == 1
                        rewExit = 1;
                        rewLatency = horzcat(rewLatency, NaN); % store nans for misses
                        [succeeded, cheetahReply] = NlxSendCommand('-PostEvent "NosePokeProbeMiss" 102 2');
                    end
                end
                cohPlotMet = vertcat(cohPlotMet,coh);
                dataMet    = vertcat(dataMet,data_det);   
                trialNum   = horzcat(trialNum,size(cohPlotMet,1));
            else
                % tracker variable
                cohMet = 0;
                % save coherence var
    ...
    %disp(['trial ',num2str(trialNum(end))])    
