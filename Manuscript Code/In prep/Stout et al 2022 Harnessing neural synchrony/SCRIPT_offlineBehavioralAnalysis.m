...
for i = 1:length(rats)
    ...
    for sessi = 1:length(dir_content)
        try
            ...
            % before moving forward, do a sanity check. We are making sure
            % that the variables line up (eg that our trial exclusions
            % above were accurate). If so, than our sequence table accuracy
            % will perfectly line up with our matlab variable tracking
            % accuracy
            if isempty(find((sequenceTable.Accuracy-accuracy')==1))==0
                error('Data is formatted improperly')
            end
