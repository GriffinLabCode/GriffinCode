function [svm_data] = svm_stem(input)
...
% first for sample left
    for ci=1:length(clusters) 
        ...
        for i = 1:length(sample_trials)
            spk_idx = find(spikeTimes>Int(sample_trials(i),1)...
                & spikeTimes<=Int(sample_trials(i),5)); 
            time    = (Int(sample_trials(i),5)-Int(sample_trials(i),1))/1e6;
...
cd(datafolder);
     ...
     for ci=1:length(clusters) 
        ...
        for i = 1:length(choice_trials)
            spk_idx = find(spikeTimes>Int(choice_trials(i),1)...
                & spikeTimes<=Int(choice_trials(i),5)); 
            time    = (Int(choice_trials(i),5)-Int(choice_trials(i),1))/1e6;
            fr(i,:)   = length(spk_idx)/time; % in Hz
        clear spk_idx time
        end
