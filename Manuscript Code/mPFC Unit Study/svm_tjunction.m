function [svm_data] = svm_tjunction(input)
...
for nn = kendog
    ...
    % define sample and choice trials index
...
% first for sample left
    for ci=1:length(clusters) 
        ...
        for i = 1:length(sample_trials)
            spk_idx = find(spikeTimes>Int(sample_trials(i),5)...
                & spikeTimes<=Int(sample_trials(i),6)); 
            time    = (Int(sample_trials(i),6)-Int(sample_trials(i),5))/1e6;
...
cd(datafolder);
     ...
     for ci=1:length(clusters) 
        ...
        for i = 1:length(choice_trials)
            spk_idx = find(spikeTimes>Int(choice_trials(i),5)...
                & spikeTimes<=Int(choice_trials(i),6)); 
            time    = (Int(choice_trials(i),6)-Int(choice_trials(i),5))/1e6;
            fr(i,:)   = length(spk_idx)/time; % in Hz
        clear spk_idx time
        end
    ...
    svm_choice = []; svm_sample=[]; svm_var_temp=[]; 
...
clearvars -except iii input stem_mean stem_sem svm_data...
    folder_names kendog Datafolders bins roc tj_mean tj_sem
