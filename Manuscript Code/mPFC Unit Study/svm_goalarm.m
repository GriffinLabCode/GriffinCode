function [svm_data] = svm_goalarm(input)
...
% first for sample left
    for ci=1:length(clusters) 
        ...
        for i = 1:length(sample_trials)
            spk_idx = find(spikeTimes>Int(sample_trials(i),6)...
                & spikeTimes<=Int(sample_trials(i),2)); 
            time    = (Int(sample_trials(i),2)-Int(sample_trials(i),6))/1e6;
...
cd(datafolder);
     ...
     for ci=1:length(clusters) 
        ...
        for i = 1:length(choice_trials)
            spk_idx = find(spikeTimes>Int(choice_trials(i),6)...
                & spikeTimes<=Int(choice_trials(i),2)); 
            time    = (Int(choice_trials(i),2)-Int(choice_trials(i),6))/1e6;
            fr(i,:)   = length(spk_idx)/time; % in Hz
        clear spk_idx time
        end
