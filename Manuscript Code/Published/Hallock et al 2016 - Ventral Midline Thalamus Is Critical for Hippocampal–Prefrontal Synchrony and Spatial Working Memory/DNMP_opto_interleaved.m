...
% This code randomly chooses a sessiontype from trial to trial (0 = no
% light, 1 = sample only, 2 = choice only, 3 = whole trial, 4 = delay
% only), starting with a no light trial.
...
%Start box
XV_sb = [40 40 160 160 40];
YV_sb = [400 200 200 400 400];
...
XV_t = [220 220 430 430 220];
YV_t = [340 310 210 480 340];
...
pulse_counter = 0;
r1 = randperm(5,5)-1;
sessiontype = r1(1,1); % Session starts with first integer of "r1"
...
for jj = 1:1000000 %Make this into a while loop (to run as long as the session is running)
    ...
    X = locationArray(2:2:length(locationArray));
    Y = locationArray(1:2:length(locationArray)-1); % Retreve position data
    
    [IN_sb,ON_sb] = inpolygon(mean(X),mean(Y),XV_sb,YV_sb);
    [IN2_sb,ON2_sb] = inpolygon(double(median(X)),double(median(Y)), XV_sb,YV_sb);  % Check to see if the rat is in the start box
    ...
    if IN_sb == 1 || ON_sb == 1 || IN2_sb == 1 || ON2_sb == 1 
        if mod(phasei,2) == 0 && phasei > 0
       sessiontype = r1(1,phasei/2+1); % If the rat is in the start box, and the last phase was a "choice" phase (i.e., the previous trial has ended),
        end
    end% Reset the phase counter to "1" (indicating the start of a new trial), and randomly choose a new sessiontype.
