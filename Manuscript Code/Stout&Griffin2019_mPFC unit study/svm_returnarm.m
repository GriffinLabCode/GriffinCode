function [svm_data] = svm_returnarm(input)
...
% first for sample left
    for ci=1:length(clusters) 
        ...
        for i = 1:length(sample_trials)
            spk_idx = find(spikeTimes>Int(sample_trials(i),7)...
                & spikeTimes<=Int(sample_trials(i),8)); 
            time    = (Int(sample_trials(i),8)-Int(sample_trials(i),7))/1e6;
...
cd(datafolder);
     ...
     for ci=1:length(clusters) 
        ...
        for i = 1:length(choice_trials)
            spk_idx = find(spikeTimes>Int(choice_trials(i),7)...
                & spikeTimes<=Int(choice_trials(i),8)); 
            time    = (Int(choice_trials(i),8)-Int(choice_trials(i),7))/1e6;
            fr(i,:)   = length(spk_idx)/time; % in Hz
        clear spk_idx time
        end
