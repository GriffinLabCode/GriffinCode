...
datafolder = strcat(datafolder, '\');    
    ...
    for ci=1:length(Clusters)
        ...
        % I added this on 8/20/19 - but this and the previous lines are
        % redundant. I could just do mean(pre_data) instead of above.
        pre_data{ci} = reshape(spike(:,:,pre_idx),[32 size(spike(:,:,pre_idx),3)])*1000000;
        post_data{ci} = reshape(spike(:,:,post_idx),[32 size(spike(:,:,post_idx),3)])*1000000;
        
        sem_pre{ci}  = std(pre_data{ci}')/(sqrt(size(pre_data{ci},1)));
        sem_post{ci} = std(post_data{ci}')/(sqrt(size(post_data{ci},1)));        
...
% shaded errors
figure('color',[1 1 1]);
varargout1 = shadedErrorBar(x_label,pre_waveform{cell_num},sem_pre{cell_num},'-r',1);
hold on;
varargout2 = shadedErrorBar(x_label,post_waveform{cell_num},sem_post{cell_num},'-b',1);
legend([varargout1.mainLine,varargout2.mainLine],'first 10 min','last 10 min')
box off
set(gca,'FontSize',12)
