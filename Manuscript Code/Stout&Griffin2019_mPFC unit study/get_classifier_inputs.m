...
function [input]=get_classifier_inputs() 
    ...
    input.rec_room = 1;
    input.numbins  = 7; % was 7 7 bins with ymax of 400 and ymin of 135 gave ~9inch bins
    ...
    if input.withinRat_design == 1
        ...
        elseif input.Prelimbic == 1
            input.rat{1} = 3:14;  % baby groot
            input.rat{2} = 15:17; % capn
            input.rat{3} = 18:22; % groot
            input.rat{4} = 23:30; % meusli
            input.rat{5} = 31:38; % thanos
            input.rat_names = char({'baby groot','capn crunch','groot',...
                'meusli','thanos'});
