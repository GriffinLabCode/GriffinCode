...
function [C,f,num_trials] = coherence_chronux_permutated(datafolder,input,Int,params)
    ...
    try
        ...
        for permi = 1:5000
            Int = [];
            % make a distribution of random numbers that are the number of
            % trials
            trialnum        = randperm(size(Int_og,1));
            % select the first two trials (permutating up to 2 trials)
            trials_included = trialnum(1:2);
            % extract
            Int = Int_og(trials_included,:);     
            ...
            %% coherence
                ...
                if input.time_freq == 1
                     [C{permi},phi,S12,S1,S2,t,f{permi},confC,phistd,Cerr]=cohgramc(data1_all,data2_all,params.movingwin,params);
                else % coherency across broad-band spectrum
                    % coherency
                    [C{permi},phi,S12,S1,S2,f{permi},confC,phistd,Cerr]=coherencyc(data1_all,data2_all,params);
                end

        % provide feedback
        fdbck = ['iteration ',num2str(permi)];
        disp(fdbck)  
        
        % report the number of trials included for each iteration
        num_trials = size(Int,1);
        end
        
        clear data1_all data2_all data1_clean data2_clean data1 data2
    catch
        disp('missing lfp')
        C = NaN;
        f = NaN;
