...
function [input]=get_coh_inputs() 
    ...
    input.simultaneous     = 0; % triple site simultaneous recordings?       
    input.T_entry_minus2   = 0; % minus two seconds
    input.Tentry_longepoch = 1; % this is -2 to 1 sec after T
    input.T_DataDriven     = 0; % this is -0.8 to 0.2 based on pfc-hpc coherence data
    input.T_beforeEffect   = 0; % this is -2 sec to -1 sec before Tentry

    input.goal_entry = 0; % goal zone entry
    ...
    input.coh_time = 1; % keep this set to 1 if you want to look in time domain 
    
    % do you want a time-frequency plot or a broad-band spectrum?
    % if you assign to 0, you get coherence X frequency spectrum
    input.time_freq = 1; % 1 is a time-frequency plot for heatmap creation
    ...
    % all my analyses looked at correct trials
    input.correct = 1;
    input.incorrect = 0;
