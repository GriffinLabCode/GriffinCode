...
datafolder = 'X:\01.Experiments\John n Andrew\Dual optogenetics w mPFC recordings\All Subjects - DNMP\Good performance\Andrew data\15-16 8-21-15';
...
%}
load(strcat(datafolder,'\Int_new.mat'));
...
for triali=1:size(Int,1)%1:size(Int,1) % trial
    ...
    time  = [(Int(triali,5)-(2*1e6)) (Int(triali,1)+(1*1e6))];
    ...
    params.Fs = SampleFrequencies(1);
    x_label = linspace(0,(size(size_data{triali},2)/params.Fs),size(size_data{triali},2));
