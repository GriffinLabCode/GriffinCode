...
load('data_numNeurons_choiceLvR_5000_EarlyStemLowRate')
...
ModelAll = fitlm(selectIdx,svm_avg,'linear'); % for pvalue
...
LogModel = fitlm(logX,logY,'linear');
check2 = round(LogModel.SSE) == round(gofModelAll.sse);
...
if isempty(find((isnan(line_data)==0)==1))==0
    for i = 1:length(futureAdded)
        y = line_data(i);
...
LogModelSS = fitlm(logX(1:numSelected),logY(1:numSelected),'linear');
