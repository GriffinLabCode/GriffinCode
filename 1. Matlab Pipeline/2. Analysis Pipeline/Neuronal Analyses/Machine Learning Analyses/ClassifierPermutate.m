...
% written by John Stout
 ...
 leave1out        = 1; % leave 1 out method for classifier - suggested
 pseudosimult     = 1; % pseudo simultaneous method
 individual       = 0; % individual neuron classification
 downsample_data  = 1; % controls for different sizes of trials - select this for permutation style classifier
 controlLeftRight = 0; % control for the number of lefts/rights? This is only important if you have DNMP task and comparing sample/choice
 firstFewTrials   = 0; % set this to 1 if you want to examine the first few trials (1:numObs), select downsample_data to 0 with this
 scale_data       = 0; % scaling data between 0 and 1 will help improve classifier accuracy
 
 % if taskphase, do 15 trials. If trajectory, do 6 trials.
 numbins = 219; % numbins will be 1 less than when you estimate it
 numObs  = 6;  % how many rows you got (ie trials). Use the minimum number. % 15 for sample/choice
 data1   = data1; % to get this, run formatData_Cleaning_...
 data2   = data2; 
 ...
 % cannot have both of these selected
 if firstFewTrials == 1 && downsample_data == 1
     downsample_data  = 0;
...
if pseudosimult == 1
    ...
    if leave1out == 1
        ...
        if downsample_data == 0 && firstFewTrials == 0
        ...
        % down-sample data to look at the first N number of trials    
        elseif downsample_data == 0 && firstFewTrials == 1
            ...
            % grab first N number of trials
            clear data1new data2new
            for clusti = 1:length(data1)
                % extract data
                data1new{clusti} = data1{clusti}(1:numObs,:);
                data2new{clusti} = data2{clusti}(1:numObs,:);             
            end
            ...
            % scale
            if scale_data == 1
                disp('Data scaled between 0 and 1')
                for bini = 1:numbins
                    data{bini} = normalize(data{bini},'range');
                    %data{bini} = zscore(data{bini});
                end
            end
        ...
        elseif leave1out == 1 && downsample_data == 1 && firstFewTrials == 0
            ...
            for n = 1:1000  
                ...
                if scale_data == 1
                    disp('Data scaled between 0 and 1')
                    for bini = 1:numbins
                        dataPerm{bini} = normalize(dataPerm{bini},'range');
                    end
                end
    ...
    if permute_labels == 1
        ...
        for bini = 1:numbins
            ...
            if downsample_data == 0
                ...
                % create labels for classifier parameters
                ones_var = ones([size(labels,1)/2,size(labels,2)]); % svm_cell deprecated
                neg_ones = -ones([size(labels,1)/2,size(labels,2)]);
            ...
            for n = 1:1000
               ...
               % svm
                ...
                if downsample_data == 1
                    data4shuff = [];
                    data4shuff = dataPerm{bini};
                else
                    data4shuff = [];
                    data4shuff = data{bini};
                end
                % svm classifier
                for i = 1:size(labels_shuff,1)
                    svm_temp = data4shuff;
                    labels_temp = labels_shuff;
                    testing_data = data4shuff(i,:);
            ...
            clearvars -except svm_perf svm_perf_rand svm_cell permute_data ...
                ...
                method_75Training_25Testing ci stat svm_std labels        
        end
...
if individual == 1
    ...
    for clusti = 1:length(data1)
        ...
        % define a dynamic variable - trials are features
        clear data
