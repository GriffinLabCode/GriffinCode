...
% the expected firing rate as a function of location must be the mean
% across trials, or maybe just the trial specific rate map. If trial
% specific, than we can train and test trial-by-trial, which is what the
% bayesian decoder does.

% expected FR  - for 1 trial for now
expectedFRs = rate_maps_pos{1}(1,:);

% bin the data into 500ms - now we gotta use time variable
tau = 500; % ms
numSamplesInTau = timeWindow*vt_srate*(1/1000); % Nms * 30 samples/sec * (1sec/1000ms) = M samples

% group neuronal activity based on tau - get data every 15 samples
% spikes = ; - some kind of for loop probably?
...
% using the avg, we should be able to get the expected fr (from the poisson
