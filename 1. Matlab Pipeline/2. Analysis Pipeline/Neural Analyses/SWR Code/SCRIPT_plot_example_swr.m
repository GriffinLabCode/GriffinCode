...
%% swr spike timing
spkTimes = get_spkTimes(datafolder,'TT');

% get all events into one array
SWRevents = horzcat(SWRtimes{:});

% onset of swr event
eventTimes = [];
for i = 1:length(SWRevents)
    eventTimes(i) = SWRevents{i}(1);
end

% find difference of event times - maybe handle the time difference stuff
% here
eventTimes_og = eventTimes;
next = 0;
while next == 0
    eventDiff = []; idxGet = [];
