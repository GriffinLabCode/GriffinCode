function [Ypk,Xpk,Wpk,Ppk] = findpeaks_MATLAB(Yin,varargin)
...
if hasX
  ...
  if FsSupplied
    % Fs
    Fs = varargin{1};
    validateattributes(Fs,{'numeric'},{'real','finite','positive'},'findpeaks','Fs');
    Fs = double(Fs(1));
    x = (0:M-1).'/Fs;
    xIsRow = yIsRow;
  else
...
elseif strcmp(wRef,'halfheight')
  ...
  iRBh = iRB;
  if length(iLBh) > 1
    iLBh(2:end) = max(iLB(2:end),iRB(1:end-1));
    iRBh(1:end-1) = min(iRB(1:end-1),iLB(2:end));  
  end
  iGuard = iLBh > iPk;
