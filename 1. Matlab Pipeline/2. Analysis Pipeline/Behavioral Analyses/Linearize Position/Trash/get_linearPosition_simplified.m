...
for i = 1:numTrials
    ...
    % time end
    time_start(i) = Int(i,Int2use(1));
    time_end(i)   = Int(i,Int2use(2));
    
    % get position data
    pos_start = dsearchn(position_data{i}(3,:)',time_start(i)');
    pos_end   = dsearchn(position_data{i}(3,:)',time_end(i)')-1;
    
    % now use the end and start positions to get linear skeleton
    actualPosStart = position_data{i}(1:2,pos_start);
    actualPosEnd   = position_data{i}(1:2,pos_end);
    
    % get linear skeleton index start and end
    idx_lin_start = dsearchn(idealTraj{i}',actualPosStart');
    idx_lin_end   = dsearchn(idealTraj{i}',actualPosEnd');
    
    % get linear skeleton start through end
    idealTrajNew{i} = idealTraj{i}(:,idx_lin_start:idx_lin_end);
     
    %figure('color','w'); plot(ExtractedX,ExtractedY,'Color',[.5 .5 .5]);
    %hold on; plot(idealTrajNew{i}(1,:),idealTrajNew{i}(2,:),'Color','r','LineWidth',2);
