...
% estimates the position data corresponding to time lags. Note this, as of
...
function [data] = get_ratLocations_DNMP(Datafolders, int_name, task_type, correct, incorrect)
    ...
    if size(Int,2) >= 9
        ...
        if task_params == 2
            if sep_traj == 'N'
                ...
                [data.sample_X{nn-2},data.sample_Y{nn-2},data.ExtractedX{nn-2},data.ExtractedY{nn-2}] = rat_location_time2pos(datafolder,Int_sample);
                [data.choice_X{nn-2},data.choice_Y{nn-2},~,~] = rat_location_time2pos(datafolder,Int_choice);
            elseif sep_traj == 'Y'
                % run rat_location function
                [data.sampleL_X{nn-2},data.sampleL_Y{nn-2},data.ExtractedX{nn-2},data.ExtractedY{nn-2}] = rat_location_time2pos(datafolder,Int_sampleL);
                [data.sampleR_X{nn-2},data.sampleR_Y{nn-2}] = rat_location_time2pos(datafolder,Int_sampleR);
                [data.choiceL_X{nn-2},data.choiceL_Y{nn-2}] = rat_location_time2pos(datafolder,Int_choiceL);  
                [data.choiceR_X{nn-2},data.choiceR_Y{nn-2}] = rat_location_time2pos(datafolder,Int_choiceR);                
            end
        end
    end
