...
public:
	...
	// They also are compiled if you have explicitly turned off refcounting
		...
		MYTYPE& assign(MYCITER iterFirst, MYCITER iterLast)
		{
			...
			if ( iterFirst < this->begin() || iterFirst > begin() + size() )
				erase()
	#endif
			replace(this->begin(), this->end(), iterFirst, iterLast);
			return *this;
		}
	#endif
	...
	{
		resize(static_cast<MYSIZE>(nNewLen > -1 ? nNewLen : sslen(this->c_str())));
	}
	...
	MYTYPE Right(int nCount) const
	{
		...
		return substr(this->size()-static_cast<MYSIZE>(nCount));
	}
	...
	MYTYPE& TrimLeft()
	{
		erase(this->begin(), std::find_if(this->begin(), this->end(),NotSpace<CT>(std::locale())));
		return *this;
	}
	...
	{
		std::locale loc;
		MYRITER it = std::find_if(this->rbegin(), this->rend(), NotSpace<CT>(loc));
		if ( this->rend() != it )
			erase(this->rend() - it);

		erase(it != this->rend() ? find_last_of(*it) + 1 : 0);
		return *this;
	}
...
//	void			UnlockBuffer(); { }	// why have UnlockBuffer w/o LockBuffer?
	...
	HRESULT StreamSave(IStream* pStream) const
	{
		...
		if ( FAILED(hr=pStream->Write(&hdr, sizeof(SSSHDR), 0)) )
			...
			;		// nothing to write
		else if ( FAILED(hr=pStream->Write(this->c_str(), size()*sizeof(CT), 0)) )
