...
ierr Processor::AllocateOuputVariables(mxArray *plhs[], int numRecs )
{
	...
	for( int i = 0; i < MaxFields; i++ ) {
		if( mFieldSelections[i] ) {
			if( IndexSamples == i ) {
				plhs[index] = mxCreateDoubleMatrix( MAX_CSC_SAMPLES, numRecs, mxREAL );	//special case for samples field b/c its a true double array
			} else {
...
//******************************************************************************************************************************************************
//******************************************************************************************************************************************************
void Processor::InsertData(CRRec* rec, int recordIndex)
{
	int index = 0;
	...
	if( mFieldSelections[IndexChannelNumbers] ) {
		mOutputVariables[index][recordIndex] = (double)( (signed __int32)(rec->dwChannelNum) );
		index++;
	}
	if( mFieldSelections[IndexSampleFrequency] ) {
		mOutputVariables[index][recordIndex] = (double)( (signed __int32)(rec->dwSampleFreq) );
		index++;
	}
	if( mFieldSelections[IndexNumberValidSamples] ) {
		mOutputVariables[index][recordIndex] = (double)( (signed __int32)(rec->dwNumValidSamples) );
		index++;
	}
	if( mFieldSelections[IndexSamples] ) {
		for( int j = 0; j < MAX_CSC_SAMPLES; j++) {
			mOutputVariables[index][j+(recordIndex*MAX_CSC_SAMPLES)] = (double)(rec->snSamples[j]);
		}
		index++;
	}
}
