...
int mFileTypeExpected = DATAFILETYPE_SESPIKE;
int mRecordSize = sizeof(SERec);
...
{
	if( nrhs == 1 ) {
		mexPrintf("\tMat2NlxSE() version 2.0\n");
		return;
	}
...
int WriteFile(std::fstream& fileHandle, BOOL* fieldSelections, int extractionMode, double* modeParameters, int numModeParameters, int numRecs)
{
	...
	for( int i = 0; i < numRecs; i++ ) {
		...
		if( fieldSelections[IndexData] ) {
			for( int n = 0; n < SE_NUMELECTRODES; n++) {
				for( int j = 0; j < SPIKE_NUMPOINTS; j++) {
					buf.snData[j].snADVal[n] =  (signed __int16)(mInputFields[index][(i*SPIKE_NUMPOINTS)+j]);
				}
			}
			index++;	
		}
