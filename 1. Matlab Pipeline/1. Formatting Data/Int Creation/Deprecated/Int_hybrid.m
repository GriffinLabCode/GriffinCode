...
% run view_VT_data
view_VT_data_autoMaze_fun(datafolder);
close all;
...
for triali = 1:size(Int,1)
    ...
    % loop across cp variable, identify when theyre in cp
    idxKeep = [];
    for i = 2:length(cpL_temp)
        if Int(triali,3) == 0
            % if they're not in left reward, but they used to be inside
            % of it, they must be leaving the reward. Also make sure they
            % aren't in the cp
            if cpL_temp(i) == 0 && cpL_temp(i-1) == 1 && cp_temp(i) == 0 && cp_temp(i+1)==0
                idxKeep = i;
                % break
                break;
            end
        elseif Int(triali,3) == 1
            if cpR_temp(i) == 0 && cpR_temp(i-1) == 1 && cp_temp(i) == 0 && cp_temp(i+1)==0
                idxKeep = i;
                % break
                break;
            end
...
percentCorrect = (numel(find(Int(2:end,4)==0))./(size(Int,1)-1))*100;
...
if contains(answer,'Y') | contains(answer,'y')
    ...
    % save
    save('Int_hybrid.mat','Int');
    disp('Saved file name "Int_hybrid"')
end
close all;
